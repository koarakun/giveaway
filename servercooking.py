...
#botの招待リンクを作成
@bot.command()
async def botlink(ctx, id):
    await ctx.send(f"discordapp.com/oauth2/authorize?client_id={id}&scope=bot&permissions=0")
...
async def janken(ctx):
    embed = discord.Embed(title="じゃんけん！！", description="最初はグーじゃんけん...", color=discord.Color.blue())

    message = await ctx.send(embed=embed)
    hands = {"👊": "グー", "🤚": "パー", "✌️": "チョキ"}

    for hand in hands:
        await message.add_reaction(hand)

    def check(reaction, user):
        return user == ctx.author and str(reaction.emoji) in hands

    try:
        reaction, _ = await bot.wait_for("reaction_add", check=check, timeout=30.0)
    except asyncio.TimeoutError:
        embed.set_footer(text="時間切れです。")
        await message.edit(embed=embed)
        return
    else:
        bot_hand = random.choice(list(hands.values()))
        user_hand = hands[str(reaction.emoji)]
        result = ""
        if user_hand == bot_hand:
            result = "引き分け！"
        elif (user_hand == "グー" and bot_hand == "チョキ") or (user_hand == "パー" and bot_hand == "グー") or (user_hand == "チョキ" and bot_hand == "パー"):
            result = "勝ち！"
        else:
            result = "負け！"

        embed.add_field(name="あなたの手", value=user_hand, inline=True)
        embed.add_field(name="botの手", value=bot_hand, inline=True)
...
#対人じゃんけん
@bot.command()
async def vsjanken(ctx, user: discord.Member):
    def check(reaction, user_check):
        return user_check == user1 or user_check == user2 and str(reaction.emoji) in hands
    ...
    # メッセージ送信
    embed = discord.Embed(title="じゃんけん！！", description="最初はグーじゃんけん...", color=discord.Color.blue())
    message1 = await user1.send(embed=embed)
    message2 = await user2.send(embed=embed)

    # リアクションを追加
    for hand in hands:
        await message1.add_reaction(hand)
        await message2.add_reaction(hand)
    ...
    try:
        ...
        user2_hand = None
        if user == user1:
            user1_hand = hands[str(reaction.emoji)]
            reaction, user = await bot.wait_for("reaction_add", check=check, timeout=30.0)
            user2_hand = hands[str(reaction.emoji)]
        elif user == user2:
            user2_hand = hands[str(reaction.emoji)]
            reaction, user = await bot.wait_for("reaction_add", check=check, timeout=30.0)
            user1_hand = hands[str(reaction.emoji)]
        ...
        if user1_hand == user2_hand:
            result = "引き分け！"
        elif (user1_hand == "グー" and user2_hand == "チョキ") or (user1_hand == "パー" and user2_hand == "グー") or (user1_hand == "チョキ" and user2_hand == "パー"):
            result = f"{user1.mention}の勝ち！"
        else:
            result = f"{user2.mention}の勝ち！"

        # 結果の送信
        embed.add_field(name=f"{user1.display_name}の手", value=user1_hand, inline=True)
        embed.add_field(name=f"{user2.display_name}の手", value=user2_hand, inline=True)
        embed.add_field(name="結果", value=result, inline=False)
...
@bot.command()
async def help(ctx):
    embed = discord.Embed(title="BOTの使い方", description="気になるコマンドのリアクションを選択してください", color=discord.Color.blue())
    embed.add_field(name="🔧 管理者用コマンド", value="管理者向けのコマンド一覧です。", inline=False)
    embed.add_field(name="🎲 フリーコマンド", value="一般ユーザー向けのコマンド一覧です。", inline=False)
...
async def on_raw_reaction_add(payload):
    ...
    if payload.emoji.name == "🔧":
        ...
        embed.add_field(
            name="`?dm`",
            value="指定したユーザーに匿名でメッセージを送信できます。形式は以下の通りです`?dm ユーザーID メッセージ`このコマンドはbotとのDmでのみ実行可能です。",
            inline=False
        )
        embed.add_field(
            name="`?ticket`",
    ...
    if payload.emoji.name == "🎲":
        embed = discord.Embed(
            title="フリーコマンドの説明",
            description="以下は、フリーコマンドの説明です。",
            color=discord.Color.blue()
        )
        embed.add_field(
            name="`?janken`",
            value="リアクションを押すことによってじゃんけんの手を選択します。60秒動作がないと時間切れになります。結構強いです",
            inline=False
        )
        embed.add_field(
            name="`?vsjanken`",
            value="対人でじゃんけんができます。コマンドの形式は以下の通りです`?vsjanken @ユーザーメンション`",
            inline=False
        )
        embed.add_field(
            name="`?3`",
